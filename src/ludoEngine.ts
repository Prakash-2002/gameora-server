import { LudoState, LudoToken, LudoPhase, LUDO_COLORS, LUDO_SAFE_TILES, getGlobalTileIndex } from './ludoTypes.js';
...
export function ludoReducer(state: LudoState, action: LudoAction, playerNames: string[]): LudoState {
  const getPlayerName = (idx: number) => playerNames[idx] || `Player ${idx}`;

  switch (action.type) {
    case 'START_LUDO': {
      const resetState = getInitialLudoState();
      return {
        ...resetState,
        gamePhase: 'rolling',
        activePlayer: 0,
    ...
    case 'ROLL_DICE': {
      ...
      let nextPhase: LudoPhase = 'moving';
      let msg = '';
    ...
    case 'MOVE_TOKEN': {
      ...
      const newTokens = state.tokens.map((pTokens, idx) => {
        if (idx === playerIndex) return playerTokens;
        return pTokens;
      });
      ...
      if (newPos >= 0 && newPos <= 50) {
        const globalIndex = getGlobalTileIndex(playerIndex, newPos);
        const isSafeTile = LUDO_SAFE_TILES.includes(globalIndex);
          ...
          for (let p = 0; p < 4; p++) {
            ...
            for (let t = 0; t < 4; t++) {
              ...
              if (ot.pathPosition >= 0 && ot.pathPosition <= 50) {
                const opponentGlobal = getGlobalTileIndex(p, ot.pathPosition);
...
export const decideLudoMove = (
  ...
  for (const tid of movableTokenIds) {
    ...
    if (newPos >= 0 && newPos <= 50) {
      const globalIndex = getGlobalTileIndex(playerIndex, newPos);
      if (globalIndex !== -1 && !LUDO_SAFE_TILES.includes(globalIndex)) {
        for (let p = 0; p < 4; p++) {
          if (p === playerIndex) continue;
          for (const ot of state.tokens[p]) {
            if (ot.pathPosition >= 0 && ot.pathPosition <= 50) {
              if (getGlobalTileIndex(p, ot.pathPosition) === globalIndex) {
  ...
  for (const tid of movableTokenIds) {
    ...
    if (token.pathPosition >= 0 && token.pathPosition <= 50) {
      const currentGlobal = getGlobalTileIndex(playerIndex, token.pathPosition);
      ...
      for (let p = 0; p < 4; p++) {
        ...
        for (const ot of state.tokens[p]) {
          if (ot.pathPosition >= 0 && ot.pathPosition <= 50) {
            const oppGlobal = getGlobalTileIndex(p, ot.pathPosition);
      ...
      if (underThreat) {
        ...
        const newGlobal = getGlobalTileIndex(playerIndex, newPos);
        if (LUDO_SAFE_TILES.includes(newGlobal) || newPos > 50) {
