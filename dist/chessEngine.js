...
exports.decideChessMove = exports.getInitialChessState = exports.getCapturedPieces = void 0;
exports.chessReducer = chessReducer;
const chess_js_1 = require("chess.js");
const getCapturedPieces = (board) => {
    const counts = {
...
const getInitialChessState = () => {
    const chess = new chess_js_1.Chess();
    const rawBoard = chess.board();
    const board = rawBoard.map((row, rIdx) => row.map((cell, cIdx) => {
        if (!cell)
            return null;
        return {
            type: cell.type,
...
exports.getInitialChessState = getInitialChessState;
function chessReducer(state, action, playerNames) {
    const getPlayerName = (idx) => playerNames[idx] || `Player ${idx + 1}`;
    switch (action.type) {
        case 'START_CHESS': {
            const initial = (0, exports.getInitialChessState)();
            return {
                ...initial,
                gamePhase: 'playing',
        ...
        case 'MAKE_MOVE': {
            ...
            try {
                ...
                const rawBoard = chess.board();
                const newBoard = rawBoard.map(row => row.map(cell => {
                    if (!cell)
                        return null;
                    return {
                        type: cell.type,
                ...
                const captured = (0, exports.getCapturedPieces)(newBoard);
        ...
        case 'RESET_CHESS': {
            return (0, exports.getInitialChessState)();
        }
        default:
            return state;
    }
}
const decideChessMove = (playerIndex, state) => {
    try {
        const chess = new chess_js_1.Chess(state.fen);
        ...
        for (const move of moves) {
            let score = 0;
            // Simulate move
            const temp = new chess_js_1.Chess(chess.fen());
            ...
            // Capture evaluations
            if (move.captured) {
                const valMap = { p: 100, n: 300, b: 300, r: 500, q: 900, k: 0 };
            ...
            if (temp.isAttacked(move.to, opponentColor)) {
                const valMap = { p: 100, n: 300, b: 300, r: 500, q: 900, k: 10000 };
...
exports.decideChessMove = decideChessMove;
