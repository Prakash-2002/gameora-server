...
exports.generateSudokuBoard = generateSudokuBoard;
exports.getInitialSudokuState = getInitialSudokuState;
exports.sudokuReducer = sudokuReducer;
exports.decideSudokuMove = decideSudokuMove;
const BASE_SOLVED = [
    5, 3, 4, 6, 7, 8, 9, 1, 2,
    6, 7, 2, 1, 9, 5, 3, 4, 8,
    1, 9, 8, 3, 4, 2, 5, 6, 7,
...
function generateSudokuBoard(cluesCount = 36) {
...
function getInitialSudokuState() {
...
function sudokuReducer(state, action, playerNames = ['Player 1', 'Player 2']) {
    const newState = {
        ...state,
        board: [...state.board],
    ...
    switch (action.type) {
        ...
        case 'MAKE_MOVE': {
            ...
            // Validate turn
            if (playerIndex !== newState.activePlayer)
                return state;
            // Validate slot boundaries & empty check
            if (cellIndex < 0 || cellIndex >= 81 || newState.board[cellIndex] !== 0)
                return state;
            if (digit < 1 || digit > 9)
                return state;
...
function decideSudokuMove(activePlayer, state) {
    const emptyIndices = [];
    for (let i = 0; i < 81; i++) {
        if (state.board[i] === 0) {
            emptyIndices.push(i);
